...
import { DlqWorker } from './dlq/dlq.worker';
...
export class QueueWorkerBootstrapService implements OnModuleInit, OnModuleDestroy {
   ...
   */
  private async startWorker(queueName: string, processor: any, options: WorkerOptions = {}) {
  ...
  private async startDlqWorker() {
    ...
    const worker = new Worker(dlqQueue.name, this.dlqWorker.process.bind(this.dlqWorker), {
      connection: this.connection.getRedis(),
      concurrency: 1,
    });
    ...
    worker.on('completed', (job: any) => {
      this.logger.debug(`DLQ Job completed: ${job.id}`);
    });
